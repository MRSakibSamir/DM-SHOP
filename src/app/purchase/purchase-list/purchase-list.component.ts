...
export class PurchaseListComponent implements OnInit {
  ...
  products: Product[] = [
    ...
    { id: 2, name: 'Farm Fresh Yogurt',   cost: 110 },
  ...
  ngOnInit(): void {
    ...
    // যদি backend থেকে supplier আনতে চান:
    // this.loadSuppliers();
  }
  ...
  addSupplier(name: string): void {
    ...
    // নতুন supplier সিলেক্ট করে দিই
    this.form.patchValue({ supplierId: nextId });

    // যদি API থাকে:
    // this.supplierService.createSupplier({ name: clean }).subscribe(newS => {
    //   this.suppliers = [...this.suppliers, newS];
    //   this.form.patchValue({ supplierId: newS.id });
    // });
  }
